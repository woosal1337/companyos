...
export function ProjectDefaultAssignee({
  orgId,
  project,
  canManage,
}: {
  ...
  return (
    <section className="flex flex-col gap-3 rounded-lg border border-border bg-surface p-4">
      <div className="flex flex-col gap-1">
        <h2 className="text-small font-semibold text-foreground">Default assignee</h2>
        <p className="text-caption text-muted-foreground">
          New work items created without an assignee are assigned to this person.
        </p>
      </div>
      <AssigneeSelect
        value={project.default_assignee_id}
        members={members}
        onChange={(userId) => updateProject.mutate({ default_assignee_id: userId })}
      />
    </section>
  );
}
