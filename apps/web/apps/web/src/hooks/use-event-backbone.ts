...
export function useOutboxEvents(orgId: string) {
  return useQuery({
    ...
    queryFn: ({ signal }) => api.get<OutboxEvent[]>(orgPath(orgId, "/events"), signal),
  });
}
...
export function useRetryEvent(orgId: string) {
  ...
  return useMutation({
    mutationFn: (eventId: string) =>
      api.post<OutboxEvent>(orgPath(orgId, `/events/${eventId}/retry`)),
    onSuccess: () => {
...
export function useDispatchEvents(orgId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: () => api.post<{ delivered: number }>(orgPath(orgId, "/events/dispatch")),
    onSuccess: (result) => {
