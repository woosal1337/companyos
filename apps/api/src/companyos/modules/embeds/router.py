...
async def unfurl(payload: UnfurlIn, ctx: OrgCtx, session: SessionDep) -> SuccessResponse[EmbedMeta]:
...
async def delete_embed(
    note_id: uuid.UUID, embed_id: uuid.UUID, ctx: OrgCtx, session: SessionDep
