...
async def test_outbox_capture_and_webhook_dispatch(
    ...
    events = await client.get(f"{API}/orgs/{org['id']}/events", headers=h)
    ...
    dispatch = await client.post(f"{API}/orgs/{org['id']}/events/dispatch", headers=h)
    assert dispatch.status_code == 200, dispatch.text
    assert dispatch.json()["data"]["delivered"] >= 1
    assert any(url == "https://example.com/hook" and sig for url, sig in captured)

    captured.clear()
    again = await client.post(f"{API}/orgs/{org['id']}/events/dispatch", headers=h)
